              ...
              <md-card-content>
                ...
                <div class="md-layout">
                  <md-field md-clearable :class="getValidationClass('nombre')">
                    <label for="first-name">Línea</label>
                    <md-input
                      name="first-name"
                      id="first-name"
                    ...
                    >El nombre de la línea es requerida</span>
...
export default {
  ...
  methods: {
    ...
    listarDatos(page, buscar, criterio) {
      let me = this;
      var url =
        "/linea?page=" + page + "&buscar=" + buscar + "&criterio=" + criterio;
      axios
        .get(url)
        .then(function(response) {
          var respuesta = response.data;
    ...
    eliminarModelo(data = []) {
      swal({
        title: "Esta seguro de Eliminar la Linea " + data["nombre"],
        type: "warning",
